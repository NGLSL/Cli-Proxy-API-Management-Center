 ...
 * Sheet 组件（v7 引入）
 *
 * 从右侧滑出的抽屉式面板，供 PluginsPage 编辑插件配置使用。
 * 与 Modal 的差异：从右侧滑入、宽度更大（md/lg/xl）、常用于复杂表单。
 *
 * 关键行为：
 *  - 通过 lockScroll 锁定 body 与 .content 容器滚动
 *  - 焦点陷井：Tab 在面板内循环，Esc 关闭（可被 closeDisabled 禁用）
 *  - 关闭时执行 280ms 滑出动画再卸载，避免内容突然消失
 *  - 支持自定义 confirmClose 钩子，用于"有未保存修改时二次确认"
 */
import {
  useCallback,
  useEffect,
...
export function Sheet({
  ...
  useEffect(() => {
    ...
    if (open) {
      ...
      // open 变 false 但 DOM 还在：播放退出动画后卸载
      queueMicrotask(() => {
        if (cancelled) return;
        startClose(false);
      });
  ...
  // 卸载时兜底清理定时器，避免 setState on unmounted
  useEffect(() => {
    return () => {
      if (closeTimerRef.current !== null) {
        window.clearTimeout(closeTimerRef.current);
