  ...
  const applyVisualChangesToYaml = useCallback(
    (currentYaml: string): string => {
      try {
        ...
        if (dirtyFields.has('quotaCacheRefreshInterval')) {
          setIntFromStringInDoc(
            doc,
        ...
        const routingDirty =
          ...
          dirtyFields.has('routingStickyTTL') ||
        ...
        if (routingDirty) {
          ...
          if (dirtyFields.has('routingStrategy')) {
            if (values.routingStrategy === 'round-robin') {
              if (docHas(doc, ['routing', 'strategy'])) doc.deleteIn(['routing', 'strategy']);
            } else {
              doc.setIn(['routing', 'strategy'], values.routingStrategy);
            }
          }
          if (dirtyFields.has('routingStrategy') || dirtyFields.has('routingStickyTTL')) {
            if (values.routingStrategy === 'sticky-round-robin') {
